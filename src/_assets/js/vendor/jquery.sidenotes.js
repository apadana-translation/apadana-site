// https://github.com/acdlite/jquery.sidenotes/blob/master/lib/jquery.sidenotes.js
  ...
  (function ($, window, document) {
    ...
    $.fn.extend({
      sidenotes: function () {
        ...
        return this.each(function () {
          ...
          $this = $(this);
          plugin =
            (_ref = $this.data("sidenotes")) != null
              ? _ref
              : $this.data("sidenotes", new SidenotesPlugin(this, option));
    ...
    SidenotesPlugin = (function () {
      SidenotesPlugin.prototype.options = {
        ...
        footnoteSeparator: ".footnotes-sep",
      ...
      function SidenotesPlugin(scope, options) {
        ...
        this.options.sidenotePlacement =
          this.options.placement || this.options.sidenotePlacement;
        if (this.options.hideFootnoteContainer) {
          this.$footnoteContainer = $(
            this.options.footnoteContainerSelector,
            scope
          );
          this.$footnoteSeparator = $(this.options.footnoteSeparator, scope);
        }
        this.$footnoteContainer =
          this.$footnoteContainer.length !== 0 ? this.$footnoteContainer : null;
        this.$postContainer = this.$footnoteContainer.parent();
        this.$footnotes = $(
          this.options.footnoteContainerSelector +
            " " +
            this.options.footnoteSelector,
          this.$postContainer
        );
        ...
        this.$footnotes.each(function () {
          ...
          sidenote = new Sidenote(
            footnoteEl,
            noRef ? null : refCounter++,
            plugin
          );
          plugin.sidenotes.push(sidenote);
          previous =
            (_ref = plugin.sidenotes[plugin.sidenotes.length - 2]) != null
              ? _ref
              : null;
      ...
      SidenotesPlugin.prototype.show = function (force) {
        ...
        if (this.isHidden || force) {
          ...
          if (
            ...
            this.options.hideFootnoteContainer
          ) {
            this.options.hide(this.$footnoteContainer);
            this.options.hide(this.$footnoteSeparator);
          }
      ...
      SidenotesPlugin.prototype.sidenotePlacement = function (
        placement,
        force
      ) {
        var group, sidenote, _i, _j, _len, _len1, _ref, _ref1;
        if (
          ...
          (placement !== this.sidenotePlacement() || force)
        ) {
          _ref = this.sidenotes;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            sidenote = _ref[_i];
    ...
    Sidenote = (function () {
      function Sidenote(footnoteEl, ref, owner) {
        ...
        this.$refMarkAnchor = $(
          "a[href='#" + escapeExpression(this.footnoteID) + "']",
          this.owner.$postContainer
        );
        this.$refMarkSup = this.$refMarkAnchor.parent("sup");
        this.$refMarkSup =
          this.$refMarkSup.length !== 0 ? this.$refMarkSup : null;
        this.isNested = $.contains(
          this.owner.$footnoteContainer.get(0),
          this.refMark().get(0)
        );
        if (this.isNested) {
        ...
        this.$sidenote = this.owner.options.formatSidenote(
          this.$footnote.html(),
          this.sidenoteID,
          this.ref
        );
        this.$pivot = !this.isNested
          ? this.refMark().parentsUntil(this.owner.$postContainer).last()
          : this.referringSidenote.$pivot;
        this.group = null;
        $backArrow = $(
          "a[href='#" + escapeExpression(this.refMarkID) + "']",
          this.$sidenote
        );
      ...
      Sidenote.prototype.refMark = function () {
        return this.$refMarkAnchor || this.$refMarkSup;
      };
    ...
    SidenoteGroup = (function () {
      ...
      SidenoteGroup.prototype.push = function (s) {
        ...
        return this.$pivot != null
          ? this.$pivot
          : (this.$pivot = this.sidenotes[0].pivot);
      };
